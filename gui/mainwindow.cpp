...
void MainWindow::doAnalyzeProject(ImportProject p, const bool checkLibrary, const bool checkConfiguration)
{
    ...
    if (mProjectFile) {
        ...
        v.clear();
        foreach (const QString &i, mProjectFile->getIgnoredIncludeDirs()) {
          v.push_back(i.toStdString());
        }
        p.ignoreIncludePaths(v);

        if (!mProjectFile->getAnalyzeAllVsConfigs()) {
            if (!mProjectFile->getVsConfig().isEmpty()) {
                p.ignoreOtherConfigs(mProjectFile->getVsConfig().toStdString());
            } else {
